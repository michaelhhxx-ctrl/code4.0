...
function setView(v, el) {
  ...
  const map = { c: ['cv', _mob ? 'flex' : 'grid'], v: ['vv', _mob ? 'flex' : 'grid'], q: ['qv', _mob ? 'flex' : 'grid'], l: ['lv', 'block'], m: ['microv', 'block'] };
...
function _syncMobilePanelDefault(v) {
  ...
  if (!bar) return;
  if (v === 'l') {
    bar.style.display = 'none';   // 算法库不需要面板条
  } else {
...
checkShareHash();
loadSavedSettings();
const _themeWasSet = !!localStorage.getItem('qedu_theme');
// 首屏显示条件：主题已设置（不是第一次打开）且本次打开不是分享链接
const _willShowHomeScreen = !!(_themeWasSet && !_hadShareHash);
loadTheme();
if (!_themeWasSet) {
  // 新用户：标记来自首页入口，确保主题→关于→指南全链路强制执行
  window._fromHomepage = true;
  showThemePicker();
} else if (!_willShowHomeScreen) {
  // 有分享 hash 时走正常 intro 流程；首屏会显示时跳过 intro
  initIntro();
}
// ── 视频分级加载 ──
