...
const Auth = {
  ...
  async login(username, password) {
    ...
    try {
      ...
      localStorage.setItem('qedu_role', data.role || 'student');
      return null;
    } catch {
  ...
  logout() {
    localStorage.removeItem('qedu_token');
    localStorage.removeItem('qedu_role');
  },

  currentRole() {
    return localStorage.getItem('qedu_role') || 'student';
  }
};
...
function updateUserUI() {
  ...
  if (teacherBtn) teacherBtn.style.display = (user && Auth.currentRole() === 'teacher') ? '' : 'none';
}
